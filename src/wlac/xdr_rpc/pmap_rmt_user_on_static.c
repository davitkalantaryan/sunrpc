	...
	u_long *port_ptr;
{
	...
	if (client != (CLIENT *)NULL) {
		...
		stat = CLNT_CALL(client, PMAPPROC_CALLIT, xdr_rmtcall_args, (caddr_t)&a,
...
#else /* 4.2 BSD */
			/* Cygwin change: pass sin_addr struct rather than
			   the s_addr member of sin_addr. */
			addrs[i++] = inet_makeaddr(inet_netof
			  (sin->sin_addr), INADDR_ANY);
#endif
		}
...
/*	baddr.sin_addr.S_un.S_addr = htonl(INADDR_ANY); */
	...
	recv_again:
		...
		readfds = mask;
		t1 = t;
		switch (select(_rpc_dtablesize(), &readfds, NULL,NULL, &t1))
		{
